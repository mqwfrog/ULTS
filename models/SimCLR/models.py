...
class ResNet_CIFAR(nn.Module):
    ...
    def __init__(self, block, layers, num_features=128, dataset_name='', reparam=False):
        ...
        self.layer_block = nn.Sequential(
            self._make_layer(block, 64, layers[0], stride=1), #2
            self._make_layer(block, 128, layers[1], stride=2), #4
            self._make_layer(block, 256, layers[2], stride=2), #4
            self._make_layer(block, 512, layers[2], stride=2) #4
        )
        # print(block.expansion)
        self.linear = nn.Linear(512 * block.expansion, num_features)
        # global SimpleNet_input_dim
        # SimpleNet_input_dim=512 * block.expansion
...
def resnet50_cifar(num_features=128, dataset_name='', reparam=False): 
    return ResNet_CIFAR(Bottleneck_CIFAR, [3, 4, 6, 3], num_features, dataset_name, reparam)

def resnet18_cifar(num_features=128, dataset_name='', reparam=False): 
    return ResNet_CIFAR(BasicBlock_CIFAR, [2, 2, 2, 2], num_features, dataset_name, reparam)


class SimpleNet(nn.Module):
    # def __init__(self, input_dim=2048, num_classes=6):
    def __init__(self, input_dim=0, num_classes=0):  
