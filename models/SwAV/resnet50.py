...
class ResNet(nn.Module):
    def __init__(
        ...
        self.linear_2 = nn.Linear(17984, 256)  

        self.conv1 = nn.Conv2d(
           in_channels, num_out_filters, kernel_size=3, stride=1, padding=1, bias=False
        ) #HAR-9,epilepsy-1
        self.bn1 = norm_layer(num_out_filters)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 64, layers[0])  # num_out_filters->64
        num_out_filters *= 2
        ...
        self.layer4 = self._make_layer(
            block, 64, layers[3], stride=2, dilate=replace_stride_with_dilation[2]
        ) #num_out_filters
